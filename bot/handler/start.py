...
async def message_everyone_add(message: types.Message):
  if message.from_user.id == ADMIN:
    await message.reply('💻Напишите название рассылки', reply_markup=keyboard_for_exiting_the_state_machine)
...
async def dev_message_everyone_bot(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        data['mailing_list_img'] = "https://www.ampersand.net/wp-content/uploads/2018/06/quotationew.jpg"
    await message.answer('☑️ рассылка отправлена', reply_markup=manu_keyboard_admins)
...
async def check_error(message: types.Message):
    ...
    await bot.send_photo(message.from_user.id, mailing_list_img, f'{mailing_list_name}\n{mailing_list_description}\n{mailing_list_sub}',\
        reply_markup=InlineKeyboardMarkup(resize_keyboard=True)\
            .add(InlineKeyboardButton(f"{text.edit_mailing_list_name}", callback_data=f"name {mailing_list_name}"))\
            .add(InlineKeyboardButton(f"{text.edit_mailing_list_description}", callback_data=f"description {mailing_list_description}"))\
            .add(InlineKeyboardButton(f"{text.edit_mailing_list_sub}", callback_data=f"sub {mailing_list_sub}"))\
...
async def message_news_add(message: types.Message):
  if message.from_user.id == ADMIN:
    await message.reply('💻Напишите название новости', reply_markup=keyboard_for_exiting_the_state_machine)
